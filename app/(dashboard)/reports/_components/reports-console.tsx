  ...
  return (
    <section className="grid page-gap">
      ...
      <section className="card panel">
        ...
        <div className="patients-table-wrap"><table className="patients-table"><thead><tr><th>Order</th><th>Patient</th><th>Results</th><th>Versions</th><th>Status</th><th>Action</th></tr></thead><tbody>{pageRows.map((row) => { const canRelease = row.total_items > 0 && row.entered_items === row.total_items; const canDownload = row.released_versions > 0; return <tr key={row.order_id}><td>{row.order_no}</td><td>{row.patient_name}</td><td>{row.entered_items}/{row.total_items}</td><td>{row.released_versions}</td><td>{row.status}</td><td><div style={{ display: "flex", gap: 6 }}><button className="button" disabled={!canRelease || busyId === row.order_id} onClick={() => onRelease(row.order_id)}>Release</button><button className="button button-secondary" disabled={!canDownload || busyId === row.order_id} onClick={() => onDownload(row.order_id)}>Download</button></div></td></tr>; })}{pageRows.length === 0 ? <tr><td colSpan={6}>No reports found.</td></tr> : null}</tbody></table></div>
