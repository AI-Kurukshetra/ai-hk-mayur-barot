    ...
    <section className="grid page-gap">
      ...
      <div className="content-grid two-col">
        ...
        <section className="card panel">
          ...
          <div className="patients-table-wrap"><table className="patients-table"><thead><tr><th>Order</th><th>Patient</th><th>Amount</th><th>Mode</th><th>Txn Ref</th></tr></thead><tbody>{pageRows.map((payment) => <tr key={payment.id}><td>{payment.order_no}</td><td>{payment.patient_name}</td><td>Rs {payment.amount.toFixed(0)}</td><td>{payment.mode}</td><td>{payment.txn_ref ?? "-"}</td></tr>)}{pageRows.length === 0 ? <tr><td colSpan={5}>No payments found.</td></tr> : null}</tbody></table></div>
